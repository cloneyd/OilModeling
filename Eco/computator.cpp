...
void Computator::setXTanPressure()
{
    ...
    createShoreBorder(m_xtan_pressure_vectors);

    QTableWidget* widget = new QTableWidget;
    widget->setRowCount(m_xtan_pressure_vectors.size());
    widget->setColumnCount(m_xtan_pressure_vectors[0].size());

    for(int i{}; i < m_xtan_pressure_vectors.size(); ++i) {
        for(int j{}; j < m_xtan_pressure_vectors[0].size(); ++j) {
            auto item = new QTableWidgetItem;
            item->setText(QString("%1").arg(m_xtan_pressure_vectors[i][j]));
            if(m_xtan_pressure_vectors[i][j] != 100001) {
...
void Computator::setYTanPressure()
{
    ...
    createShoreBorder(m_ytan_pressure_vectors);

           QTableWidget* widget = new QTableWidget;
           widget->setRowCount(m_ytan_pressure_vectors.size());
           widget->setColumnCount(m_ytan_pressure_vectors[0].size());

           for(int i{}; i < m_ytan_pressure_vectors.size(); ++i) {
               for(int j{}; j < m_ytan_pressure_vectors[0].size(); ++j) {
                   auto item = new QTableWidgetItem;
                   item->setText(QString("%1").arg(m_ytan_pressure_vectors[i][j]));
                   if(m_ytan_pressure_vectors[i][j] != 100001) {
                       item->setBackground(Qt::cyan);
                   }
                   widget->setItem(i, j, item);
               }
...
void Computator::setRotT()
{
    ...
    createShoreBorder(m_rot_t_vectors);

           QTableWidget* widget = new QTableWidget;
           widget->setRowCount(m_rot_t_vectors.size());
           widget->setColumnCount(m_rot_t_vectors[0].size());

           for(int i{}; i < m_rot_t_vectors.size(); ++i) {
               for(int j{}; j < m_rot_t_vectors[0].size(); ++j) {
                   auto item = new QTableWidgetItem;
                   item->setText(QString("%1").arg(m_rot_t_vectors[i][j]));
                   if(m_rot_t_vectors[i][j] != 100001) {
                       item->setBackground(Qt::cyan);
                   }
                   widget->setItem(i, j, item);
               }
...
void Computator::setF0() {
    ...
    for(int i{}; i < rows; ++i) {
        for(int j{}; j < cols; ++j) {
           ...
           QTableWidget* widget = new QTableWidget;
           widget->setRowCount(m_f0_vectors.size());
           widget->setColumnCount(m_f0_vectors[0].size());

           for(int i{}; i < m_f0_vectors.size(); ++i) {
               for(int j{}; j < m_f0_vectors[0].size(); ++j) {
                   auto item = new QTableWidgetItem;
                   item->setText(QString("%1").arg(m_f0_vectors[i][j]));
                   if(m_heights[i][j].first) {
                       item->setBackground(Qt::cyan);
                   }
                   widget->setItem(i, j, item);
